...
/* Funkcja uruchamiana, gdy napotkamy sygnał TSTP a*/
void on_tstp(int signal) {
        printf("Otrzymałem TSTP, ale nie przeniosę procesu do bg\n");
}
...
int main (int argc, char** argv) {
	...
	/* Konfiguracja obsługi sygnału TSTP */
        struct sigaction tstp;
        sigemptyset(&mask); /* Wyczyść maskę */
        tstp.sa_handler = (&on_tstp);
        tstp.sa_mask = mask;
